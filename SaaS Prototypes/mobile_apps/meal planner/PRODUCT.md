...
## Product Direction
- Prototype label: meal planner
- Intended product: meal planner
- Current code reality: shared VelocityCore customer-operations scaffold
- Value proposition: Provide a runnable starting point for teams exploring a meal planner product direction while making the scaffold status explicit.

## Target Users
- Builders evaluating a meal planner product direction
- Operators or founders who need a code-first starting point before domain-specific implementation

## Personas
- Founder or product lead validating a future meal planner product line.
- Operator who wants billing, communication, workflow, and reporting seams before deep domain implementation.
- Engineer responsible for replacing the shared scaffold with the first real meal planner workflow.
...
- meal planner domain entities are still missing and must replace or extend the shared models before this becomes a true product.
...
- Replace the generic flows above with the first true meal planner workflow.
...
### Free
- Audience: Builders evaluating the meal planner prototype locally.
...
### Pro
- Audience: Small teams piloting a hosted meal planner workflow.
...
3. Replace one generic scaffold workflow with a real meal planner workflow.
...
- First milestone: Replace the generic customer-operations data model and UI with meal planner-specific behavior.
