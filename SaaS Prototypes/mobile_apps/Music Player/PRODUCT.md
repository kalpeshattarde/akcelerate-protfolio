...
## Product Direction
- Prototype label: Music Player
- Intended product: Music Player
- Current code reality: shared VelocityCore customer-operations scaffold
- Value proposition: Provide a runnable starting point for teams exploring a Music Player product direction while making the scaffold status explicit.

## Target Users
- Builders evaluating a Music Player product direction
- Operators or founders who need a code-first starting point before domain-specific implementation

## Personas
- Founder or product lead validating a future Music Player product line.
- Operator who wants billing, communication, workflow, and reporting seams before deep domain implementation.
- Engineer responsible for replacing the shared scaffold with the first real Music Player workflow.
...
- Music Player domain entities are still missing and must replace or extend the shared models before this becomes a true product.
...
- Replace the generic flows above with the first true Music Player workflow.
...
### Free
- Audience: Builders evaluating the Music Player prototype locally.
...
### Pro
- Audience: Small teams piloting a hosted Music Player workflow.
...
3. Replace one generic scaffold workflow with a real Music Player workflow.
...
- First milestone: Replace the generic customer-operations data model and UI with Music Player-specific behavior.
