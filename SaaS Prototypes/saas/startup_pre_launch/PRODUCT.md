...
## Product Direction
- Prototype label: startup_pre_launch
- Intended product: startup_pre_launch
- Current code reality: shared VelocityCore customer-operations scaffold
- Value proposition: Provide a runnable starting point for teams exploring a startup_pre_launch product direction while making the scaffold status explicit.

## Target Users
- Builders evaluating a startup_pre_launch product direction
- Operators or founders who need a code-first starting point before domain-specific implementation

## Personas
- Founder or product lead validating a future startup_pre_launch product line.
- Operator who wants billing, communication, workflow, and reporting seams before deep domain implementation.
- Engineer responsible for replacing the shared scaffold with the first real startup_pre_launch workflow.
...
- startup_pre_launch domain entities are still missing and must replace or extend the shared models before this becomes a true product.
...
- Replace the generic flows above with the first true startup_pre_launch workflow.
...
### Free
- Audience: Builders evaluating the startup_pre_launch prototype locally.
...
### Pro
- Audience: Small teams piloting a hosted startup_pre_launch workflow.
...
3. Replace one generic scaffold workflow with a real startup_pre_launch workflow.
...
- First milestone: Replace the generic customer-operations data model and UI with startup_pre_launch-specific behavior.
