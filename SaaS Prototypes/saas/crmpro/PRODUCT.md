...
## Product Direction
- Prototype label: crmpro
- Intended product: crmpro
- Current code reality: shared VelocityCore customer-operations scaffold
- Value proposition: Provide a runnable starting point for teams exploring a crmpro product direction while making the scaffold status explicit.

## Target Users
- Builders evaluating a crmpro product direction
- Operators or founders who need a code-first starting point before domain-specific implementation

## Personas
- Founder or product lead validating a future crmpro product line.
- Operator who wants billing, communication, workflow, and reporting seams before deep domain implementation.
- Engineer responsible for replacing the shared scaffold with the first real crmpro workflow.
...
- crmpro domain entities are still missing and must replace or extend the shared models before this becomes a true product.
...
- Replace the generic flows above with the first true crmpro workflow.
...
### Free
- Audience: Builders evaluating the crmpro prototype locally.
...
### Pro
- Audience: Small teams piloting a hosted crmpro workflow.
...
3. Replace one generic scaffold workflow with a real crmpro workflow.
...
- First milestone: Replace the generic customer-operations data model and UI with crmpro-specific behavior.
