...
## Product Direction
- Prototype label: Workout Trainer
- Intended product: Workout Trainer
- Current code reality: shared VelocityCore customer-operations scaffold
- Value proposition: Provide a runnable starting point for teams exploring a Workout Trainer product direction while making the scaffold status explicit.

## Target Users
- Builders evaluating a Workout Trainer product direction
- Operators or founders who need a code-first starting point before domain-specific implementation

## Personas
- Founder or product lead validating a future Workout Trainer product line.
- Operator who wants billing, communication, workflow, and reporting seams before deep domain implementation.
- Engineer responsible for replacing the shared scaffold with the first real Workout Trainer workflow.
...
- Workout Trainer domain entities are still missing and must replace or extend the shared models before this becomes a true product.
...
- Replace the generic flows above with the first true Workout Trainer workflow.
...
### Free
- Audience: Builders evaluating the Workout Trainer prototype locally.
...
### Pro
- Audience: Small teams piloting a hosted Workout Trainer workflow.
...
3. Replace one generic scaffold workflow with a real Workout Trainer workflow.
...
- First milestone: Replace the generic customer-operations data model and UI with Workout Trainer-specific behavior.
