...
## Product Direction
- Prototype label: job_board
- Intended product: job_board
- Current code reality: shared VelocityCore customer-operations scaffold
- Value proposition: Provide a runnable starting point for teams exploring a job_board product direction while making the scaffold status explicit.

## Target Users
- Builders evaluating a job_board product direction
- Operators or founders who need a code-first starting point before domain-specific implementation

## Personas
- Founder or product lead validating a future job_board product line.
- Operator who wants billing, communication, workflow, and reporting seams before deep domain implementation.
- Engineer responsible for replacing the shared scaffold with the first real job_board workflow.
...
- job_board domain entities are still missing and must replace or extend the shared models before this becomes a true product.
...
- Replace the generic flows above with the first true job_board workflow.
...
### Free
- Audience: Builders evaluating the job_board prototype locally.
...
### Pro
- Audience: Small teams piloting a hosted job_board workflow.
...
3. Replace one generic scaffold workflow with a real job_board workflow.
...
- First milestone: Replace the generic customer-operations data model and UI with job_board-specific behavior.
