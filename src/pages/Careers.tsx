  ...
  return (
    <>
      ...
      <section className="py-20"><div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="space-y-4">{jobs.map((j, i) => (
          <div key={i} className="glass-card p-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div><h3 className="font-poppins font-semibold text-lg">{j.title}</h3><p className="text-muted-foreground text-sm mt-1">{j.desc}</p>
                <div className="flex gap-4 mt-3"><span className="text-xs flex items-center gap-1 text-muted-foreground"><MapPin className="w-3 h-3" />{j.location}</span><span className="text-xs flex items-center gap-1 text-muted-foreground"><Briefcase className="w-3 h-3" />{j.type}</span></div>
              </div>
