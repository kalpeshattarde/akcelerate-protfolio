...
function getUsers(): UserRecord[] {
  try { return JSON.parse(localStorage.getItem("ak-users") || "[]"); } catch { return []; }
}
...
function getSalesAsUsers(): UserRecord[] {
  try {
    ...
    for (const order of orders) {
      ...
      if (!map.has(email)) {
        map.set(email, { id: email, email, name: order.name || email.split("@")[0], signupDate: order.date || new Date().toISOString(), purchases: 0, totalSpent: 0, lastActive: order.date || new Date().toISOString(), status: "active" });
      }
...
export default function UserManagementTab() {
  ...
  const users = useMemo(() => {
    ...
    return stored.length > 0 ? stored : getSalesAsUsers();
  }, []);
  ...
  const exportUsers = () => {
    downloadCSV("users-export.csv",
      ["Name", "Email", "Purchases", "Total Spent", "Status", "Last Active"],
      filtered.map(u => [u.name, u.email, String(u.purchases), `$${u.totalSpent}`, u.status, new Date(u.lastActive).toLocaleDateString()])
    );
  };
